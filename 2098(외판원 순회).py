...
n = int(input())
w = []
dp = [[0 for _ in range(n)] for i in range(2**n)]
INF = float('inf')
...
def travel(visited, cur):
    ...
    if visited == (1 << n) - 1: # 모든 도시를 방문한 경우
        if w[cur][0] != 0: # 출발 도시로 갈 수 있는 경로가 있는 경우 -> 사이클 때문에 어느 도시에서 출발하든 최소 비용은 같다. 따라서 출발 도시는 0번 도시로 한다. 
            return w[cur][0]
        else:
            return INF
    
    if dp[visited][cur] != 0: # dp[현재까지 방문한 도시들][현재 도시] -> 현재 도시에서 출발 도시까지 가는 데 필요한 비용 
